<%--
 - $Header: /home/cvsroot/mvnforum_enterprise/srcweb/mvnplugin/mvnforum/im/lobbychatx.jsp,v 1.14 2009/03/11 09:11:54 trungth Exp $
 - $Author: trungth $
 - $Revision: 1.14 $
    ...
    <script type="text/javascript">
      ...
      function updateData(data) {
        ...
        for (i = 0; i < datas.length-1; i++) {
          ...
          //alert("dataParts[0].contains('<')" + dataParts[0].contains("<"));
          //alert("dataParts[0].contains('<')" + dataParts[0].contains("<"));
          //if (dataParts[1].contains("<") == false && dataParts[2].contains("undefined") == false) {
            //alert("hi");
      ...
      function sendMessage() { 
        if (CheckForm() == true) {
      var url = "<%=urlResolver.encodeURL(request, response, "sendlobbymessage")%>";
      ...
      function getLogMessage() {
        test = true;
        //alert('getLogMessage()');
        var url = "<%=urlResolver.encodeURL(request, response, "getlobbymessage")%>";
        return url;
      }

      function refresh() {
          alert('refresh()');
          var url = "<%=urlResolver.encodeURL(request, response, "refreshlobby")%>";
...
<table style="width:100%; height:15px;" cellspacing="0" cellpadding="0" class="noborder">
    <tr style="width:100%; background-color: rgb(115, 166, 255);">
      <td width="15" align="left">
        <img src="<%=contextPath%>/mvnplugin/mvnforum/images/chat/available_dkblue1.gif" />
       </td>
      <td class="receiver">
        <span>lobbychat</span>
       </td>
...
<table style="width:100%;" cellspacing="0" cellpadding="0" class="noborder">
    <tr style="margin-bottom:2px;">
      <td align="center" class="input">
        <div>
          <textarea id="data" name="data" onKeyPress="checkEnter(event);" class="message"></textarea>
        </div>
      </td>
    </tr>
    ...
    <tr style="width:100%; height:30px;">
      <td>
      <div>
        <span class="whatever" style="float: left; margin-left: 2px;" name="submitbutton" id="send" onmouseover="mouseOver();" onmouseout="mouseOut();" onclick="sendRequest(sendMessage()); document.getElementById('data').value = '';"> Send </span> 
      </div>
