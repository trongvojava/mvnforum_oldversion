...
//va="contentNiceName,email,reg-emailaddress,reg-emailaddress-two,reg-password,reg-password-check,UserName,PassWord,PassWord_Check,reg-name,reg-members-display-name,members_display_name,reg_code,EmailAddress,EmailAddress_two".split(',') //Put the ID of the fields you DON'T want to let users type Vietnamese in, multiple fields allowed, separated by a comma (,)
method=2 //Default input method, 0=AUTO, 1=TELEX, 2=VNI, 3=VIQR
on_off=1 //Start AVIM on
dockspell=1 //Start AVIM with spell checking on
dauCu=0 //Start AVIM with old way of marking accent (o`a, o`e, u`y)
...
function findIgnore(el) {
	for(i=0;i<va.length;i++) if((el.id==va[i])&&(va[i].length>0)&&(el.name==va[i])) return true
}
