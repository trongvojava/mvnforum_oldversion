<%--
  - $Header: /home/cvsroot/mvnforum_enterprise/srcweb/mvnplugin/mvnforum/admin/editbannedusernamex.jsp,v 1.46 2009/08/13 10:31:09 thonh Exp $
  - $Author: thonh $
  - $Revision: 1.46 $
...
<mvn:head>
  <mvn:title><fmt:message key="mvnforum.common.forum.title_name"/> - <fmt:message key="mvnforum.admin.editbannedusernamex.title"/></mvn:title>
...
<div class="nav center">
  ...
  <a href="<%=urlResolver.encodeURL(request, response, "listbannedusernames")%>"><fmt:message key="mvnforum.admin.listbannedusernamesx.title"/></a>&nbsp;&raquo;&nbsp;
  <fmt:message key="mvnforum.admin.editbannedusernamex.title"/>
</div>
<br/>

<%
BannedUsernameBean bannedUsername = (BannedUsernameBean)request.getAttribute("BannedUsername");
%>

<form action="<%=urlResolver.encodeURL(request, response, "editbannedusernameprocess", URLResolverService.ACTION_URL)%>" method="post" name="submitform">
<%=urlResolver.generateFormAction(request, response, "editbannedusernameprocess")%>
<mvn:securitytoken />
<input type="hidden" name="usernameID" value="<%=bannedUsername.getId()%>" />
<table class="tborder" width="95%" cellspacing="0" cellpadding="3" align="center">
<mvn:cssrows>
  <tr class="portlet-section-header">
    <td colspan="2"><fmt:message key="mvnforum.admin.editbannedusernamex.title"/>:</td>
  </tr>
  <tr class="<mvn:cssrow/>">
    ...
    <td><input type="text" id="username" name="username" value="<%=bannedUsername.getBannedUsernameText()%>" onkeyup="initTyper(this);" size="50" /></td>
  </tr>
  <tr class="<mvn:cssrow/>">
    <td><label for="note"><fmt:message key="mvnforum.admin.listbannedusernamesx.note"/><span class="requiredfield"> *</span></label></td>
    <td><input type="text" id="note" name="note" value="<%=bannedUsername.getNote() %>" onkeyup="initTyper(this);" size="50"/></td>
  </tr>
  <%
  boolean neverExpire = (bannedUsername.getCreationDate().compareTo(bannedUsername.getExpireDate()) == 0);
  %>
  <tr class="<mvn:cssrow/>">
        ...
        <tr>               
          <td <%if (neverExpire) {%>style="display: none"<%} else {%> style="display:"<%}%> id="chooseexpiredate">
            <%if (neverExpire == false) {
                ...
                request.setAttribute("TimeToDisplay", bannedUsernameExpireDate);
              }  
            %>
