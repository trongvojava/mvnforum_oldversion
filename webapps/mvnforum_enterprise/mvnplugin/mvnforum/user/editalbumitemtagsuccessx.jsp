<%--
  - $Header: /home/cvsroot/mvnforum_enterprise/srcweb/mvnplugin/mvnforum/user/editalbumitemtagsuccessx.jsp,v 1.8 2009/10/22 09:54:19 hoanglt Exp $
  - $Author: hoanglt $
  - $Revision: 1.8 $
...
AlbumItemBean albumItemBean = (AlbumItemBean) request.getAttribute("AlbumItemBean"); 
%>

<fmt:bundle basename="i18n/mvnforum/mvnForum_i18n">
<mvn:html locale="${currentLocale}">
<mvn:head>
  <mvn:title><fmt:message key="mvnforum.common.forum.title_name"/> - <fmt:message key="mvnforum.user.editalbumitemtagsuccessx.title"/></mvn:title>
<%@ include file="/mvnplugin/mvnforum/meta.jsp"%>
<meta http-equiv="refresh" content="3; url=<%=urlResolver.encodeURL(request, response, "viewalbumitem?itemid=" + albumItemBean.getAlbumItemID())%>" />
...
<div class="nav center">
  ...
  <fmt:message key="mvnforum.common.albumitem"/>: <a href="<%=urlResolver.encodeURL(request, response, "viewalbumitem?itemid=" + albumItemBean.getAlbumItemID())%>">
  <% if (albumItemBean.getItemTitle().length() > 0) {%>
      <%=albumItemBean.getItemTitle() %></a>&nbsp;&raquo;&nbsp;
    <%} else {%>
      <fmt:message key="mvnforum.common.albumitem.untitled"/></a>&nbsp;&raquo;&nbsp;
    <%}%>
  <fmt:message key="mvnforum.user.editalbumitemtagsuccessx.title"/>
</div>
<br/>
...
<mvn:cssrows>
  ...
  <tr class="<mvn:cssrow/>">
    <td>
      <b>&raquo;&nbsp;</b>
      <fmt:message key="mvnforum.common.albumitem"/>: <a class="command" href="<%=urlResolver.encodeURL(request, response, "viewalbumitem?itemid=" + albumItemBean.getAlbumItemID())%>">
        <% if (albumItemBean.getItemTitle().length() > 0) {%>
          <%=albumItemBean.getItemTitle() %></a>
        <%} else {%>
