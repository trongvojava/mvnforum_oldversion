<%--
  - $Header: /home/cvsroot/mvnforum_enterprise/srcweb/mvnplugin/mvnforum/user/viewthreadtreex.jsp,v 1.31 2009/11/18 08:12:08 minhnn Exp $
  - $Author: minhnn $
  - $Revision: 1.31 $
...
Collection postBeanTree = (Collection)request.getAttribute("postBeanTree");
...
int memberPostsPerPage = 10000;//onlineUser.getPostsPerPage();
int firstPostID = ((Integer)request.getAttribute("firstPostID")).intValue();
String posts[] = (String[]) request.getAttribute("postsArray");
...
<table width="95%" align="center">
  <tr>
    <td valign="bottom">
      ...
      <% } %>
      <span class="highlight" name="totalPost"><fmt:message key="mvnforum.user.viewthread.total_posts"/>: <%=numberOfPosts%></span>
...
<%-- @todo: Isn't there canDeleteThread() ? --%>
    ...
    <a class="command" href="<%=urlResolver.encodeURL(request, response, "movethread?thread=" + threadID)%>"><img src="<%=imagePath%>/button_move_thread.gif" border="0" alt="<fmt:message key="mvnforum.user.viewthread.link.move_thread"/>" title="<fmt:message key="mvnforum.user.viewthread.link.move_thread"/>"/></a><br/>&nbsp;
<%}%>
<%if (MVNForumConfig.getEnableEasyWatching() && onlineUser.isMember()) { %>
 ...
 <% } %>
    <a class="command" href="<%=urlResolver.encodeURL(request, response, "addfavoritethreadprocess?thread=" + threadID, URLResolverService.ACTION_URL)%>"><img src="<%=imagePath%>/button_add_favorites.gif" border="0" alt="<fmt:message key="mvnforum.user.viewthread.link.add_favorite_thread"/>" title="<fmt:message key="mvnforum.user.viewthread.link.add_favorite_thread"/>"/></a>&nbsp;
<%} // if online user %>
    <a class="command" href="<%=urlResolver.encodeURL(request, response, "viewthread?thread=" + threadID)%>"><img src="<%=imagePath%>/button_flat_mode.gif" border="0" alt="<fmt:message key="mvnforum.user.viewthread.link.view_thread_flat"/>" title="<fmt:message key="mvnforum.user.viewthread.link.view_thread_flat"/>"/></a>&nbsp;
...
<mvn:cssrows>
<table id="tree_navigation" class="tborder" align="center" cellpadding="5" cellspacing="0" width="95%">
  <tr class="portlet-section-alternate">
    <td valign="top" width="100%">
      <div style="height:180px; width:100%; overflow: auto;">
<%
boolean isFirst = true;
%>  <%=request.getAttribute("postTree")%>
  ...
  <tr class="portlet-section-footer">
    <td align="center" colspan="2">
      <input name="expandbutton" class="liteoption" value="Expand all" onclick="javascript:expandAll();" type="button">
      <input name="collapsebutton" class="liteoption" value="Collapse all" onclick="javascript:collapseAll();" type="button">
    </td>
  </tr>
</table>
<br/>
...
<%} //end of popup menu %>
<pg:item>
<span id="post<%=postBean.getPostID()%>" <% if (isFirst == false) { %>style="display:none"<%} %> /> 
<% isFirst = false; %>
        ...
        <tr>
          <td width="100%" colspan="2" valign="top" class="messageTextBold" align="left">
            ...
            <span id="posttopic_<%=postBean.getPostID()%>">
              <%=MyUtil.filter(postBean.getPostTopic(), false/*html*/, false/*emotion*/, true/*mvnCode*/, false/*newLine*/, false/*URL*/)%>
            </span>
          </td>
        </tr>   
      </table>
...
function showPost(ID) { 
  if (tempid != 0) {
    ...
    postToShow = document.getElementById("post"+ID);
  if (postToShow == null ) {
    //load content via ajax
    loadPostContent(ID);
  } else { //data is already loaded, just go ahead
    document.getElementById("post"+ID).style.display="block";
    var currentOjb = "highlight"+ID ;
    document.getElementById(currentOjb).className="thread-tree-view-selected";
    $('placeHolder').style.visibility = 'hidden';
    //window.location = "#tree_navigation";
    //document.getElementById('loaderContainer').style.visibility = 'hidden';
  }
<%
if (enablePost) {
%>
  var topic = document.getElementById("posttopic_" + ID).innerHTML;
  if (document.getElementById("input")) {
    document.getElementById("quickreplymessage").innerHTML = "<fmt:message key="mvnforum.user.viewthread.quickreply_to_post"/> [" + ID + ", " + topic + "]";
  }
  if (topic.indexOf("Re: ") == -1) {
    topic = "Re: " + topic;
  }
  document.mvnform.parent.value=ID;
  document.mvnform.PostTopic.value=topic;
<%
}
%>
...
function hidePost(ID) { 
  ...
  var currentOjb = "highlight"+ID ;
  document.getElementById(currentOjb).className="thread-tree-view-alternate";
}
function umbala(ID) { 
  var view = document.getElementById("view"+ID);
  if (view.style.display == "block") { 
    view.style.display = "none";
    document.getElementById("nav" + ID).src = "<%=contextPath%>/mvnplugin/mvnforum/images/tree/tree_nav_h_green.gif";
  } else {
    view.style.display = "block";
    document.getElementById("nav" + ID).src = "<%=contextPath%>/mvnplugin/mvnforum/images/tree/tree_nav_v_brown.gif";
  }
}
function expandAll() {
  var posts = new Array(<%for (int i = 1; i<posts.length-1; i++) {out.print("\""+ posts[i] +"\",");} out.print("\""+posts[posts.length-1]+"\"");%>);
  for (i = 0; i< posts.length; i++) {
    document.getElementById("nav" + posts[i]).src = "<%=contextPath%>/mvnplugin/mvnforum/images/tree/tree_nav_v_brown.gif";
    document.getElementById("view" + posts[i]).style.display = "block";
  }
}
function collapseAll() {
  var posts = new Array(<%for (int i = 1; i<posts.length-1; i++) {out.print("\""+ posts[i] +"\",");} out.print("\""+posts[posts.length-1]+"\"");%>);
  for (i = 0; i< posts.length; i++) {
    document.getElementById("nav" + posts[i]).src = "<%=contextPath%>/mvnplugin/mvnforum/images/tree/tree_nav_h_green.gif";
    document.getElementById("view" + posts[i]).style.display = "none";
  }
}
function loadPostContent(postID) {
  var loaderContainer = $('loaderContainer');
  //loaderContainer.style.height = document.viewport.getHeight();
  loaderContainer.style.visibility = 'visible';
  $('placeHolder').style.visibility = 'visible';
  var url = "loadpostcontent?postID=" + encodeURIComponent(postID);
...
<div id="loaderContainer" style="position: fixed; top: 0px; left:47%; visibility: hidden;">
<span style="background: green; color: white"><b>&nbsp;Loading ...&nbsp;</b></span>
</div> 
<div id="placeHolder" style="visibility: hidden">
